(function () {
  ...
  const translations = {
    en: { title: 'AtlasStock', products: 'Products', name: 'Name', sku: 'SKU', category: 'Category', qty: 'Qty', price: 'Price', add: 'Add', edit: 'Edit', del: 'Delete', import: 'Import CSV', export: 'Export CSV', total: 'Total items', low: 'Low stock', activity: 'Activity log', stockValue: 'Stock Value', skuCount: 'Unique SKUs', clear: 'Clear DB', save: 'Save', cancel: 'Cancel', newproduct: 'New product', search: 'Search...' },
    fr: { title: 'AtlasStock', products: 'Produits', name: 'Nom', sku: 'SKU', category: 'Catégorie', qty: 'Quantité', price: 'Prix', add: 'Ajouter', edit: 'Éditer', del: 'Supprimer', import: 'Importer CSV', export: 'Exporter CSV', total: 'Total', low: 'Stock faible', activity: 'Journal', stockValue: 'Valeur stock', skuCount: 'SKU uniques', clear: 'Vider BD', save: 'Enregistrer', cancel: 'Annuler', newproduct: 'Produit', search: 'Rechercher...' },
    ar: { title: 'أطلس ستوك', products: 'المنتجات', name: 'الاسم', sku: 'رمز', category: 'الفئة', qty: 'الكمية', price: 'السعر', add: 'إضافة', edit: 'تعديل', del: 'حذف', import: 'استيراد CSV', export: 'تصدير CSV', total: 'إجمالي العناصر', low: 'نفاد المخزون', activity: 'سجل النشاط', stockValue: 'قيمة المخزون', skuCount: 'SKU مميزة', clear: 'مسح قاعدة البيانات', save: 'حفظ', cancel: 'إلغاء', newproduct: 'منتج جديد', search: 'بحث...' }
  };
  ...
  function injectControls() {
    ...
    toolbar.style.cssText = 'width:100%;max-width:1100px;padding:8px;display:flex;gap:8px;align-items:center;justify-content:flex-start;margin:6px auto';
  ...
  function translateUI() {
    ...
    const ths = { name: document.getElementById('th-name'), qty: document.getElementById('th-qty'), price: document.getElementById('th-price') };
    if (ths.name) ths.name.textContent = i18n('name'); if (ths.qty) ths.qty.textContent = i18n('qty'); if (ths.price) ths.price.textContent = i18n('price');
    if (qEl) qEl.placeholder = i18n('search');
    const bAdd = document.querySelector('[data-i18n="add"]'); if (bAdd) bAdd.textContent = i18n('add');
    const modTitle = document.getElementById('injectedModalTitle'); if (modTitle) modTitle.textContent = i18n('newproduct');
    document.querySelectorAll('[data-i18n]').forEach(el => { const key = el.getAttribute('data-i18n'); if (key) el.textContent = i18n(key); });
  }
  ...
  async function renderAll() {
    ...
    if (productListTbody) {
      ...
      list.forEach((it, idx) => {
        ...
        const tdSku = document.createElement('td'); tdSku.textContent = it.sku || '';
        const tdCat = document.createElement('td'); tdCat.textContent = it.category || '';
        const tdQty = document.createElement('td'); tdQty.textContent = Number(it.qty) || 0; if (Number(it.qty) <= Number(it.threshold || 0)) tdQty.className = 'low';
        const tdPrice = document.createElement('td'); tdPrice.textContent = (Number(it.price) || 0).toFixed(2) + ' ' + (curSel ? curSel.value : '');
        const tdUpdated = document.createElement('td'); tdUpdated.textContent = it.updated ? new Date(it.updated).toLocaleString() : '';
        const tdActions = document.createElement('td');
        const edit = document.createElement('button'); edit.textContent = i18n('edit'); edit.className = 'btn ghost'; edit.style.marginRight = '6px'; edit.addEventListener('click', () => openInjectedModalWith(it));
        const del = document.createElement('button'); del.textContent = i18n('del'); del.className = 'btn ghost'; del.addEventListener('click', () => { if (confirm(i18n('del') + '?')) deleteProductDB(it.id); });
        tdActions.appendChild(edit); tdActions.appendChild(del);
        tr.appendChild(tdIdx); tr.appendChild(tdName); tr.appendChild(tdSku); tr.appendChild(tdCat); tr.appendChild(tdQty); tr.appendChild(tdPrice); tr.appendChild(tdUpdated); tr.appendChild(tdActions);
  ...
  async function onInjectedFormSubmit(e) { e.preventDefault(); const p = { name: document.getElementById('in_name').value.trim(), qty: Number(document.getElementById('in_qty').value) || 0, price: Number(document.getElementById('in_price').value) || 0, category: document.getElementById('in_category').value.trim() || '' }; if (editingInjected) { await updateProductDB(editingInjected, p); } else { await addProductDB(p); } closeInjectedModal(); }
  ...
  document.addEventListener('keydown', function (e) {
    if (e.key === 'n' && !e.metaKey && !e.ctrlKey && !e.shiftKey) { e.preventDefault(); openInjectedModal(); }
    if (e.key === '/' && document.activeElement.tagName !== 'INPUT') { e.preventDefault(); if (qEl) qEl.focus(); }
  ...
  (async function init() {
    injectControls(); populateLanguages(); populateCurrencies(); const lang = detectLanguage(); if (langSel) langSel.value = lang; const cur = detectCurrency(); if (curSel) curSel.value = cur; translateUI();
    ...
    const fileInput = document.getElementById('injectedFile'); if (fileInput) fileInput.addEventListener('change', onInjectedFileChange);
    const injectedModal = document.getElementById('injectedModal'); if (!injectedModal) createInjectedModal();
  })();

  function createInjectedModal() {
